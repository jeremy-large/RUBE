...
class FakeDataGenerator(Generator):
    def __init__(self, batch_size, neg_samples, context_size, max_quantity, n_samples, stock_vocab,
                 generate_users=False, user_vocab_size=None, seed=None, **kwargs):
        if seed is not None:
            np.random.seed(seed)
        data = self.build_samples(n_samples, len(stock_vocab), user_vocab_size, context_size, max_quantity,
                                  generate_users)
        super(FakeDataGenerator, self).__init__(data, batch_size, neg_samples, max_quantity,
                                                stock_vocab=stock_vocab, user_vocab_size=user_vocab_size,
                                                seed=seed, **kwargs)

    def build_samples(self, n_samples, stock_vocab_size, user_vocab_size, context_size, max_quantity, generate_users):
        ...
        if generate_users:
            u = np.random.randint(low=0, high=user_vocab_size, size=(n_samples, 1))
            return {'q': q, 'p': p, 'u': u}
        else:
            return {'q': q, 'p': p}